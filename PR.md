...
| No logic changes                  | **One deliberate exception, made in response to review.** `CurrentTimeIndicator` and the mount auto-scroll now compute from `userTimezone` rather than browser-local time — a behaviour change, and the right one, because the new chip made a pre-existing timezone bug user-visible. Nothing else: no data fetching, auth, validation or business logic touched. |
...
All rows above pass as of the review-fix commit. The two destructive confirmations that originally fired without a pending state — the bulk-delete in `TaskPanel` and the delete-calendar in `CalendarList` — were flagged in review and are now guarded, so the `/calendar` row moved from **gap** to pass.
...
4. ~~The now-line's vertical position uses browser-local time while the hour gutter labels use the user's stored timezone.~~ **Fixed in review.** I had shipped this as a documented residual on the grounds that fixing it was a logic change. Two reviewers pushed back, and they were right: the pre-existing bug was an invisible few-pixel offset, but adding a chip turned it into a clock showing the wrong time next to the wrong row. `CurrentTimeIndicator` now takes `userTimezone`, and the position, the chip and the mount auto-scroll all derive from it via the same `formatInTimeZone(..., 'HH:mm')` call `getTimePosition` already used for event blocks. Verified across four timezones.
