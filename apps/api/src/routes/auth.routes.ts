 ...
 */
auth.get('/auth/oauth/google', async (c) => {
 ...
 */
auth.get('/auth/oauth/google/callback', async (c) => {
  ...
  if (!code || !state || !storedState || !storedCodeVerifier || state !== storedState) {
    logger.warn({ state, storedState: !!storedState }, 'Google OAuth: invalid state');
 ...
 */
auth.get('/auth/oauth/github', async (c) => {
  const state = generateState();
 ...
 */
auth.get('/auth/oauth/github/callback', async (c) => {
  const code = c.req.query('code');
  const state = c.req.query('state');
  ...
  if (!code || !state || !storedState || state !== storedState) {
    logger.warn({ state, storedState: !!storedState }, 'GitHub OAuth: invalid state');
    return c.redirect(`${FRONTEND_URL}/login?error=oauth_failed`);
  }
